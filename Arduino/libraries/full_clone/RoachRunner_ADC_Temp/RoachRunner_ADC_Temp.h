...
class CS5532 {
public:
	CS5532();//byte& _cs, byte& _sdof, byte& _esdo, byte& _osc, MUX& _mux);
	void declare(byte _cs, byte _sdof, byte _esdo, byte _osc, MUX _mux);
	void init(void);
	...
	void command8all(uint8_t);
	void hold8(uint8_t);
	void hold8all(uint8_t);
	void command16(uint16_t);
	void command32(void);
	void command32(uint32_t);
	void command32all(void);
	void command32all(uint32_t);
	void scan(void);
	uint32_t quickScan(void);
	void scanMeas(void);
	float scanMeas(int);
	void wordSplit(uint32_t);
	void readReg(reg);
	void writeReg(reg);
	void writeRegAll(reg);
	//void sdoFlag(void);
