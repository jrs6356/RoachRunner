...
//CS5532::CS5532(byte& _cs, byte& _sdof, byte& _esdo, byte& _osc, MUX& _mux) : cs(_cs), sdof(_sdof), esdo(_esdo), osc(_osc), mux(_mux) {
CS5532::CS5532(){
...
void CS5532::declare(byte _cs, byte _sdof, byte _esdo, byte _osc, MUX _mux){
	cs = _cs;
	sdof = _sdof;
	esdo = _esdo;
	osc = _osc;
...
void CS5532::init(){
	...
	*/
	//Serial.println(sdof);
	//pinMode(sdof, INPUT_PULLUP);
	//attachInterrupt(digitalPinToInterrupt(sdof), sdoFlag, FALLING);
	
	if(!SPIStarted){
		startSPI();
		//pinMode(sdof, INPUT_PULLUP);
		//attachInterrupt(digitalPinToInterrupt(sdof), raiseFlag, FALLING);
		//Serial.print("\nSPI Started\n");
	}
	//pinMode(sdof, INPUT);
	//if(cs>0 && !pinsSet){
...
void CS5532::contMeas(reg r){
	...
	if(_go){
		...
		//comB.rolecall();
		updateConv();
		command8all(comB.out);
		//Serial.println("Cont meas quick scan:");
		//quickScan();
...
void CS5532::command8all(uint8_t val){
	//digitalWrite(cs, HIGH);
	mux.out(31,1);
...
void CS5532::hold8all(uint8_t val){
	//digitalWrite(cs, HIGH);
	mux.out(31,1);
	SPI.transfer(val);
}
...
void CS5532::command32all(uint32_t val){
	wordSplit(val);
	//digitalWrite(cs, HIGH);
	mux.out(31,1);
...
uint32_t CS5532::quickScan(){
	...
	//mux.out(esdo,0);
	//Serial.println("Quick scan");
	while(!flag){}
	timestamp = micros();
	//Serial.println(digitalRead(sdof));
	//delay(1);
	//}
