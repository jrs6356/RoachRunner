...
const int NUM_BUFFERS = 50;  // Number of buffers to send as a chunk
const int BUFFER_SIZE = sizeof(unsigned long) + sizeof(float) + sizeof(int) * 12 + 1;
struct ADCSamples {
  unsigned long t;
  float f;
  int m[12];
};
ADCSamples adcSamplesA[NUM_BUFFERS];
ADCSamples adcSamplesB[NUM_BUFFERS];
volatile bool newDataAvailableA = false;
volatile bool newDataAvailableB = false;
...
TaskHandle_t prepareTaskHandle;
TaskHandle_t sendTaskHandle;
void prepareTask(void* pvParameters);
...
void setup(){
  ...
  xTaskCreate(prepareTask, "PrepareTask", 300, NULL, 2, &prepareTaskHandle);
  vTaskSuspend(prepareTaskHandle);
  xTaskCreate(sendTask, "SendTask", 300, NULL, 1, &sendTaskHandle);
...
void loop() {
  if(verboseFSM){
    SerialUSB.println("-------------------------------------------------------");
    SerialUSB.print("Page: ");SerialUSB.println(page);
  }
  btnF = false;
  scroll = true;
  scrollText = false;
...
void prepareTask(void* pvParameters) {
  ...
  while(1){
    ...
    if(!newDataAvailableA){
      // Prepare new data in outputBuffers
      for(int i=0; i<NUM_BUFFERS; i++) {
        // Assign new data to the buffer elements
        k+=1;
        sdoF = false;
        for(int j=0;j<12;++j){
          sc[j]->adc.scanMeas();
        ...
        sdoF = false;
        adcSamplesA[i].t = sc0.adc.timestamp;
        adcSamplesA[i].f = k/((float)sc0.adc.timestamp/pow(10.0,6));
    
        // Fill the int array with new values
        for(int j=0; j<12; j++) {
          adcSamplesA[i].m[j] = sc[j]->adc.conv2Dec();
        }
      }
      newDataAvailableA = true;
    }
    else if(!newDataAvailableB){
      // Prepare new data in outputBuffers
      for(int i=0; i<NUM_BUFFERS; i++) {
        // Assign new data to the buffer elements
        k+=1;
        ...
        sdoF = false;
        adcSamplesB[i].t = sc0.adc.timestamp;
        adcSamplesB[i].f = k/((float)sc0.adc.timestamp/pow(10.0,6));
    
        // Fill the int array with new values
        for(int j=0; j<12; j++) {
          adcSamplesB[i].m[j] = sc[j]->adc.conv2Dec();
        }
      }
      newDataAvailableB = true;
    }
  }
}
...
void sendTask(void* pvParameters) {
  while(1) {
    ...
    if(newDataAvailableA){
      // Send all output buffers as a single chunk
      int sTot = NUM_BUFFERS*sizeof(ADCSamples) + NUM_BUFFERS;
      uint8_t* tempBuffer = new uint8_t[sTot];
      int offset = 0;
      for(int i=0;i<NUM_BUFFERS;++i){
        memcpy(&tempBuffer[offset], &adcSamplesA[i], sizeof(ADCSamples));
        offset += sizeof(ADCSamples);
        tempBuffer[offset++] = '\n';  // Add the end marker '\n'
      }
      SerialUSB.write(tempBuffer,sTot);
      delete[] tempBuffer;
      newDataAvailableA = false;
    }
    else if(newDataAvailableB){
      // Send all output buffers as a single chunk
      int sTot = NUM_BUFFERS*sizeof(ADCSamples) + NUM_BUFFERS;
      ...
      int offset = 0;
      for(int i=0;i<NUM_BUFFERS;++i){
        memcpy(&tempBuffer[offset], &adcSamplesB[i], sizeof(ADCSamples));
        offset += sizeof(ADCSamples);
        tempBuffer[offset++] = '\n';  // Add the end marker '\n'
      }
      SerialUSB.write(tempBuffer,sTot);
      delete[] tempBuffer;
      newDataAvailableB = false;
    }
  }
}
...
pages ptrial(){
  ...
  for(int i=0;i<12;++i){
    ...
    SerialUSB.print(sc[i]->name);
    SerialUSB.print(": Gia="+pgiaG[sc[i]->pgiaG]);
    SerialUSB.print(", Gadc="+adcG[sc[i]->adc.setup1.stpVals[2]]);
    SerialUSB.print(", Fs="+fs[sc[i]->adc.setup1.stpVals[3]]+" Sps");
    SerialUSB.print(", Ro=");
    SerialUSB.print(sc[i]->adc.offsetReg1.offset,8);
    SerialUSB.print(", Rg=");
    SerialUSB.println(sc[i]->adc.gainReg1.gain,8);
  }
  ...
  while(!key){
    ...
    if(!key){
      ...
      tstart = micros();
      measure = true;
      //xTaskCreate(prepareTask, "PrepareTask", 1000, NULL, 1, &prepareTaskHandle);
      //vTaskSuspend(prepareTaskHandle);
      //xTaskCreate(sendTask, "SendTask", 1000, NULL, 2, &sendTaskHandle);
      //vTaskSuspend(sendTaskHandle);
      //vTaskStartScheduler();
      ...
      while(!scroll){
        for(int k=0;k<nBuf;k++){
          ...
          sdoF = false;
          if(togs[2]){
            adcSamples[k].t = sc0.adc.timestamp;
          }
          adcSamples[k].f = j/((float)(sc0.adc.timestamp-tstart)/pow(10.0,6));
          switch(adcU){
            case 0:
              for(int i=0;i<12;++i){
                adcSamples[k].m[i] = m[i];
              }
              break;
            case 1:
              for(int i=0;i<12;++i){
                adcSamples[k].m[i] = m[i]>>8;
              }
              break;
            case 2:
              for(int i=0;i<12;++i){
                adcSamples[k].m[i] = sc[i]->adc.conv2Dec();
              }
              break;
            case 3:
              for(int i=0;i<12;++i){
                adcSamples[k].m[i] = ((float)(sc[i]->adc.conv2Dec()))/pow(2.0,23);
              }
              break;
            case 4:
              for(int i=0;i<12;++i){
                adcSamples[k].m[i] = sc[i]->adc.conv2F();
              }
              break;
          }
          // / *
          if(togs[1]){
            ...
            output+= ",";
            output+= String(a.acceleration.x,5);
            output+= ",";
            output+= String(a.acceleration.y,5);
            output+= ",";
            output+= String(a.acceleration.z,5);
          }
          if(togs[0]){
            output+= ",";
            output+= String((bldc.MEAS_SPEED())/mc);
          }
          // * /
            //SerialUSB.println(output);
            //memcpy(b2, &adcSamples, sizeof(ADCSamples));
          //memcpy(b3,&adcSamples.t,sizeof(adcSamples.t));
          //memcpy(b3+sizeof(adcSamples.t),&adcSamples.f,sizeof(adcSamples.f));
          //memcpy(b3+sizeof(adcSamples.t)+sizeof(adcSamples.f),adcSamples.m,sizeof(adcSamples.m));
          //b3[sizeof(b3)-1] = '\0';
            //SerialUSB.write(b2,sizeof(ADCSamples));
          //SerialUSB.write(b3,sizeof(b3));
  
          //SerialUSB.write(reinterpret_cast<const uint8_t*>(&adcSamples.t), sizeof(adcSamples.t));
          //SerialUSB.write(reinterpret_cast<const uint8_t*>(&adcSamples.f), sizeof(adcSamples.f));
          //SerialUSB.write(reinterpret_cast<const uint8_t*>(adcSamples.m), sizeof(adcSamples.m));
        
          // Send the end-of-buffer marker
          //SerialUSB.write(END_MARKER);
        }
        uint8_t* tempBuffer = new uint8_t[sTot];
        int offset = 0;
        for(int i=0;i<nBuf;i++){
          memcpy(&tempBuffer[offset], &adcSamples[i], sizeof(ADCSamples));
          offset += sizeof(ADCSamples);
          tempBuffer[offset++] = END_MARKER;  // Add the end marker '\n'
        }
        SerialUSB.write(tempBuffer,sTot);
        delete[] tempBuffer;
      }
...
bool setOFF(){
  ...
  for(int i=0;i<12;++i){
    ...
    if(verboseFSM){
      SerialUSB.println(sc[i]->adc.gainReg1.out);
    }
  }
  ...
  *(pr[1]) = roll;
  ...
  for(int i=0;i<12;++i){
    ...
    for(int j=0;j<50;++j){
      ...
      if(verboseFSM){
        ...
        if(accelTog){
          ...
          roll = (atan(aclU[1]/aclU[2])/PI)*180.0;
          SerialUSB.print(" , ");
          SerialUSB.print(aclMeas[j][0],4);
          SerialUSB.print(" , ");
          SerialUSB.print(aclMeas[j][1],4);
          SerialUSB.print(" , ");
          SerialUSB.print(aclMeas[j][2],4);
          SerialUSB.print("\t->\t");
          SerialUSB.print(aclU[0],6);
          SerialUSB.print(" , ");
          SerialUSB.print(aclU[1],6);
          SerialUSB.print(" , ");
          SerialUSB.print(aclU[2],6);
          SerialUSB.print("\t->\t");
          SerialUSB.print("Pitch: ");
          SerialUSB.print(pitch,4);
          SerialUSB.print(" , Roll: ");
          SerialUSB.println(roll,4);
        }
        else{
          SerialUSB.print("\n");
        }
    ...
    if(verboseFSM){
      SerialUSB.print("Means: ");
      SerialUSB.print((uint32_t)ocalsM[i]);
    }
    ocalsmF[i]=0.0;
    for(int k=0;k<31;++k){
    ...
    if(verboseFSM){
      SerialUSB.print(" (");
      SerialUSB.print(ocalsmF[i],8);
      SerialUSB.println(")");
      SerialUSB.print(omin[i]);
      SerialUSB.print(", ");
      SerialUSB.print(omax[i]);
      SerialUSB.print(", ");
      SerialUSB.print(orng[i]);
      if(orng[i]==0){
        SerialUSB.println("\tERROR!! REDO");
      }
      else{
        SerialUSB.print("\n");
      }
    ...
    *cfgCals[i] = sc[i]->adc.offsetReg1.out;
    ...
    if(verboseFSM){
      SerialUSB.print("\nOffset1 Reg: ");
      SerialUSB.println(sc[i]->adc.offsetReg1.out);
    }
  }
  updateFlash();
  ...
  *(pr[1]+ax+1) = roll;
  ...
  for(int i=ax*4;i<(ax+1)*4;++i){
    ...
    for(int j=0;j<50;++j){
      ...
      if(verboseFSM){
        SerialUSB.print(gcals[j][i]);
        gcalsmF[i]=0.0;
        for(int k=0;k<30;++k){
            gcalsmF[i] = gcalsmF[i]+((1&(gcals[j][i]>>k))*(pow(2.0,k-24)));
        }
        SerialUSB.print("\t(");
        SerialUSB.print(gcalsmF[i],6);
        SerialUSB.print(")");
        if(accelTog){
          mag = sqrt(pow(aclMeas[i][0],2)+pow(aclMeas[i][1],2)+pow(aclMeas[i][2],2));
          aclU[0] = aclMeas[j][0]/mag;
    ...
    if(verboseFSM){
      SerialUSB.print("Means: ");
      SerialUSB.print((uint32_t)gcalsM[i]);
    }
    gcalsmF[i]=0.0;
    for(int k=0;k<30;++k){
    ...
    if(verboseFSM){
      SerialUSB.print(" (");
      SerialUSB.print(gcalsmF[i],8);
      SerialUSB.println(")");
      SerialUSB.print(gmin[i]);
      SerialUSB.print(", ");
      SerialUSB.print(gmax[i]);
      SerialUSB.print(", ");
      SerialUSB.print(grng[i]);
      if(grng[i]==0){
        SerialUSB.println("\tERROR!! REDO");
      }
      else{
...
void sdoF_isr(){
  if(!sdoF){
    if(syncADCs){
      ...
      //SerialUSB.println("-- SDOF 0 Flipped --");
      sdoF = true;
    }
  }
}
...
void sdo1F_isr(){
  if((!syncADCs&&!sdoF)&&(adcMic==1)){
    //SerialUSB.println("-- SDOF 1 Flipped --");
    sdoF = true;
  }
}
...
void sdo2F_isr(){
  if((!syncADCs&&!sdoF)&&(adcMic==2)){
    //SerialUSB.println("-- SDOF 2 Flipped --");
    sdoF = true;
  }
}
...
void sdo3F_isr(){
  if((!syncADCs&&!sdoF)&&(adcMic==3)){
    //SerialUSB.println("-- SDOF 3 Flipped --");
    sdoF = true;
  }
}
...
void sdo4F_isr(){
  if((!syncADCs&&!sdoF)&&(adcMic==4)){
    //SerialUSB.println("-- SDOF 4 Flipped --");
    sdoF = true;
  }
}
...
void sdo5F_isr(){
  if((!syncADCs&&!sdoF)&&(adcMic==5)){
    //SerialUSB.println("-- SDOF 5 Flipped --");
    sdoF = true;
  }
}
...
void sdo6F_isr(){
  if((!syncADCs&&!sdoF)&&(adcMic==6)){
    //SerialUSB.println("-- SDOF 6 Flipped --");
    sdoF = true;
  }
}
...
void sdo7F_isr(){
  if((!syncADCs&&!sdoF)&&(adcMic==7)){
    //SerialUSB.println("-- SDOF 7 Flipped --");
    sdoF = true;
  }
}
...
void sdo8F_isr(){
  if((!syncADCs&&!sdoF)&&(adcMic==8)){
    //SerialUSB.println("-- SDOF 8 Flipped --");
    sdoF = true;
  }
}
...
void sdo9F_isr(){
  if((!syncADCs&&!sdoF)&&(adcMic==9)){
    //SerialUSB.println("-- SDOF 9 Flipped --");
    sdoF = true;
  }
}
...
void sdo10F_isr(){
  if((!syncADCs&&!sdoF)&&(adcMic==10)){
    //SerialUSB.println("-- SDOF 10 Flipped --");
    sdoF = true;
  }
}
...
void sdo11F_isr(){
  if((!syncADCs&&!sdoF)&&(adcMic==11)){
    //SerialUSB.println("-- SDOF 11 Flipped --");
    sdoF = true;
  }
}
...
void initFlash(){
  ...
  if(firstRun){
    if(verboseFSM){
      SerialUSB.println("Allocating Flash Storage for Configuration");
    }
...
void updateFlash(){
  if(verboseFSM){
    SerialUSB.println("\nUploading Configuration to Flash");
  }
...
void loadFlash(){
  if(verboseFSM){
    SerialUSB.println("Downloading Configuration from Flash");
  }
